...
date: 2026-01-30 16:16:00 +0800
slug: SpringBoot-AOP
permalink: /post/SpringBoot-AOP.html
---
...
### ![代理对象示例图](/assets/posts/SpringBoot-AOP/image-20260130160542937.png)
...
![Log类示例](/assets/posts/SpringBoot-AOP/image-20260130161236251.png)

在需要增强的方法上加上该注解。

![在方法上增加该注解](/assets/posts/SpringBoot-AOP/image-20260130161352651.png)

表达式写法：`"@annotation(com.duckling.anno.Log)"`。

![AOP类中表达式声明书写](/assets/posts/SpringBoot-AOP/image-20260130161533270.png)
...
3. 解析请求头中的 JWT 令牌。
4. 获取用户 ID。
5. 调用 `BaseContext.setCurrentId(id)` **存入** ThreadLocal。
6. **关键点**：在 `finally` 块或 `afterCompletion` 中调用 `BaseContext.remove()` **清理**数据。
7. **AOP 层**：
8. 直接调用 `BaseContext.getCurrentId()` 获取当前操作人的 ID，填入日志对象。
